...
from .geozone import EARTHDIR, GeoZone,EARTHREP
...
class Ocli(GeoZone):
    ...
    def __init__(self, rawdir=FCOVERDIR, geomap="map.geojson"):
        ...
        self.revision = [self.df.REVISION[indice] for indice in self.df.index]
    ...
    def FCOVER (self,name):
        """
        Créée un fichier FCOVERtablesNomZone.csv avec les données les fichers .nc
        """
        FCOVERPATH = EARTHDIR + 'FCOVERtable'+name.replace(" ","")+'.csv'
        headers = ['DATE', 'FILE','REVISION', 'FCOVER']
        os.makedirs(os.path.dirname(FCOVERPATH), exist_ok=True)
        with open(FCOVERPATH, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(headers)
            for n in self.names:
                if n == name:
                    for d in self.dates:
                        indice = self.dates.index(d)
                        RT = self.revision[indice]
                        F = self.values(d,n)
                        m = np.nanmean(F)
                        fname = self.ncfilebydate(d)
                        writer.writerow([d,fname,RT,f"{m}"])
    ...
    def plot(self,date,name):
        ...
        plt.imshow(F)
        m = np.nanmean(F)
        plt.title(name + " le " + date + " avec un " + f"taux de couverture végétale moyen : {100*m:.1f}%.");
